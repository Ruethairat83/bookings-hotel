...
//                    WHERE 1 AND t1.hotel_id = {$_GET["id"]} AND amount != 0
//                        AND NOT EXISTS
//                        (SELECT *
...
//                         WHERE 1
//                            AND ( DATE(st2.check_in) < {$_GET["checkin"]}
//                            OR DATE(st2.check_out) > '{$_GET["checkout"]}'))";
    $sqlRoomType = "SELECT *
                    FROM room_types
                    WHERE 1 AND hotel_id = {$_GET["id"]} AND amount != 0
                    ";
        ...
        <div class="col-12 col-md-7">
           ...
           <div class="row">
                <div class="col-12">
                    ...
                    <button type="button" class="btn btn-primary mt-3" onclick="comment();" 
                    <?php echo isset($_SESSION["user_data"]) ? "" : "disabled"; ?>>
                    <?php echo isset($_SESSION["user_data"]) ? "บันทึก" : "กรุณาเข้าสู่ระบบก่อนบันทึกคอมเมนต์"; ?>
                    </button>
                </div>
           </div>
        </div>
...
<script>
    ...
    function comment() {
        if ($("#comment").val()) {
            ...
            formData.append("hotel_id", <?php echo $_GET["id"]; ?>);

            $.ajax({
                type: "POST",
                url: "./functions/insert-comment-ajax.php",
                data: formData,
                contentType: false,
                cache: false,
                dataType: "json",
            ...
            Swal.fire({
                title: 'Comment',
                text: "กรุณากรอกข้อความที่ต้องการบันทึก",
                icon: 'warning',
                confirmButtonText: "ตกลง",
                cancelButtonText: "ยกเลิก",
                showCancelButton: false,
